...
import { Search, Loader2, Check, X, Clock, ChevronDown, ChevronRight, Mail } from 'lucide-react'
...
//
// approval.original_email stores the initial outreach email (HTML) that was
// sent to the lead, captured by the scenario from activities.summary where
// activity_type = 'email_sent'. It's shown here read-only, collapsed by
// default, so reviewers have the full context without cluttering the card.
...
// Collapsed by default — reviewers usually only need the reply + draft,
// but the original pitch is one click away for context.
function OriginalEmail({ html }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Mail size={13} style={{ color: 'var(--ink-400)' }} />
        Original email sent
      </button>
      ...
      {open && (
        <div
          ...
          // Safe: this content is our own outreach copy generated by the
          // outbound campaign, not third-party or user-submitted input.
...
export default function ApprovalPage({ showToast }) {
  ...
  const fetchRows = useCallback(async () => {
    ...
    // '*' already includes original_email — no extra column needed here.
            ...
            {paginate(filtered, page).map(row => {
              ...
              return (
                <div key={row.id} style={{ border: '1px solid var(--line)', borderRadius: 12, padding: 16, background: 'var(--surface)' }}>
                  ...
                  <OriginalEmail html={row.original_email} />
